/**
 * Shared types for the Referral Copilot UI.
 *
 * These exactly mirror the JSON the Express bridge returns from the Python
 * scoring pipeline. Keep them in lock-step with
 * `python_bridge/referral_cli.py::_row_to_candidate` and the contract
 * documented in `docs/referral_ui_integration_plan.md`.
 *
 * The frontend renders these as-is — never recompute scores client-side.
 */
...
export interface ReferralSearchParams {
  ...
  user_lat: number;
  user_lon: number;
  /** RankingPriority for known cases; `string` keeps room for future enums. */
...
export interface ReferralParseResponseOk {
  ...
  /** LLM-assessed urgency score 1–10. Display hint only — does not filter or route candidates. */
  urgency_score?: number;
...
export type ReferralParseResponse = ReferralParseResponseOk | ReferralParseResponseClarify | ReferralBridgeError;
