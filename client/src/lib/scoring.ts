...
function computeConditionScore(facility: FacilityNode, condition: string): number {
  ...
  const text = [facility.specialties, facility.capability, facility.procedure, facility.equipment, facility.description]
    .filter(Boolean)
    .join(' ')
    .toLowerCase();
...
function computeTrustScore(facility: FacilityNode): number {
  ...
  const richFields = [facility.specialties, facility.capability, facility.procedure, facility.equipment].filter(Boolean)
    .length;
  ...
  return Math.round(100 * (0.4 * infoScore + 0.35 * capScore + 0.25 * ageScore));
}
...
function computeEvidenceScore(facility: FacilityNode): number {
  ...
  return Math.min(
    100,
    richFields * 15 + descBonus + (facility.sourceCount ? Math.min(20, facility.sourceCount * 4) : 0)
  );
}
